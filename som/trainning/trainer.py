...
class Trainer:
    def __init__(self, cfg_path: str, resume_ckpt: str = None):
        ...
        if self.local_rank == 0:
            wandb.init(
                entity=self.cfg.get('train', {}).get('wandb_entity', 'storm'),
                project=self.cfg.get('train', {}).get('experiment_name', 'som_training'),
                config=self.cfg,
                name=f"{self.cfg['train'].get('running_name', 'default')}",
                resume="allow"
            )
    ...
    def _build_dataset(self):
        ...
        self.train_ds = MultiDataset(dc['root'], split='train', image_size=size, num_views=num_views, use_augment=True, include_datasets=include_datasets)
    ...
    @torch.no_grad()
    def validate_epoch(self, step: int = None) -> float:
        self.model.eval()
        total_iou, cnt = 0.0, 0
    ...
    def run(self):
        ...
        while step < max_steps:
            ...
            if step % val_interval == 0 or step == max_steps:
                ...
                val_prec, val_recall, val_ap = self.validate_epoch(step)
